...
public class FilteringService : IFilteringService
{
    ...
    {
        if (payPerHour < 0) return query;
        query = query.Where(p => p.Job.Salary != null && (
            (p.Job.Salary.SalaryRate == SalaryRates.PerHour &&
             ((p.Job.Salary.MaximalAmount != null && p.Job.Salary.MaximalAmount >= payPerHour) ||
              p.Job.Salary.MinimalAmount >= payPerHour)) ||
            (p.Job.Salary.SalaryRate == SalaryRates.PerDay &&
             ((p.Job.Salary.MaximalAmount != null && p.Job.Salary.MaximalAmount >= payPerDay) ||
              p.Job.Salary.MinimalAmount >= payPerDay)) ||
            (p.Job.Salary.SalaryRate == SalaryRates.PerWeek &&
             ((p.Job.Salary.MaximalAmount != null && p.Job.Salary.MaximalAmount >= payPerWeek) ||
              p.Job.Salary.MinimalAmount >= payPerWeek)) ||
            (p.Job.Salary.SalaryRate == SalaryRates.PerMonth &&
             ((p.Job.Salary.MaximalAmount != null && p.Job.Salary.MaximalAmount >= payPerMonth) ||
              p.Job.Salary.MinimalAmount >= payPerMonth)) ||
            (p.Job.Salary.SalaryRate == SalaryRates.PerYear &&
             ((p.Job.Salary.MaximalAmount != null && p.Job.Salary.MaximalAmount >= payPerYear) ||
              p.Job.Salary.MinimalAmount >= payPerYear))
        ));
        return query;
    }
    ...
    public IQueryable<ProcessedJobWord> FilterByLocation(IQueryable<ProcessedJobWord> query, string location)
    {
        return !string.IsNullOrEmpty(location)
            ? query.Where(p => p.Job.Location.ToLower().Contains(location.ToLower())
                               || p.Job.LocationCountry.ToLower().Contains(location.ToLower()))
            : query;
    }
    ...
    public IQueryable<ProcessedJobWord> FilterByLanguage(IQueryable<ProcessedJobWord> query, string language)
    {
        return !string.IsNullOrEmpty(language)
            ? query.Where(p => p.Job.Language.ToLower() == language.ToLower())
            : query;
    }
}