...
{
    private CreateResumeValidator()
    {
        RuleFor(r => r.Educations).NotEmpty()
            .When(r => r.WorkExperiences.Count == 0)
            .WithMessage("You didn't provide enough information to create resume");
    ...
    public static void ValidateCreatedResume(CreateResumeDTO resume)
    {
        ...
        if (!validationResult.IsValid) throw new InvalidResumeException(validationResult.ToString());
