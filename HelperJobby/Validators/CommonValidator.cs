...
public class CommonValidator
{
    ...
    {
        if (string.IsNullOrEmpty(email)) throw new ValidationException("You can not have an empty email");

        if (email.Length < 4 || email.Length > 50) throw new ValidationException("Length of your Email is invalid");
    ...
    {
        if (string.IsNullOrEmpty(phoneNumber)) throw new ValidationException("You can not have an empty phoneNumber");

        if (phoneNumber.Length < 5 || phoneNumber.Length > 15)
            throw new ValidationException("Length of your phone number is invalid");

        var regexPattern = @"^\+[0-9]{1,3}[0-9]{3,14}$";
